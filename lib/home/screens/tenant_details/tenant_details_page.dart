import 'package:bomatrack/home/screens/tenant_details/bloc/tenant_details_page_bloc.dart';
...
class _TenantDetailsState extends State<TenantDetails> {
                                ...
                                onPressed: state is TenantDetailsPageLoading
                                    ...
                                    : () {
                                        if (formKey.currentState!.validate()) {
                                          final monthlyRent = double.parse(rentController.text.trim());
                                          ...
                                          context.read<TenantDetailsPageBloc>().add(
                                            AddUnitPressed(
                                              ...
                                              startDate: startDate.toString().split(' ')[0], // Format as YYYY-MM-DD
                                            ),
                                          );
                                        }
                                      },
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    Text('Are you sure you want to remove Unit ${unit.unitNumber}?'),
                    const SizedBox(height: 16),
                    TextFormField(
                        ...
                        return SizedBox(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: state is TenantDetailsPageLoading
                                ...
                                : () {
                                    if (formKey.currentState!.validate()) {
                                      final newRentText = newRentController.text.trim();
                                      final newRent = double.tryParse(newRentText);
  ...
  void _addPayment() {
    ...
    if (state is HomeLoaded && state.payments.isNotEmpty) {
      ...
      for (var payment in pendingPayments) {
        String paymentType = payment.description?.toLowerCase().contains('deposit') ?? false
            ? 'deposit'
            : 'rent';
    ...
    groupedPayments.forEach((key, payments) {
      if (payments.length > 1) {
        ...
        for (var payment in payments) {
          final tenancy = _tenancies.firstWhere((t) => t.id == payment.unitTenancyId);
        ...
        consolidatedPendingPayments.add(Payment(
          ...
          description: payments.first.description?.toLowerCase().contains('deposit') ?? false
            ...
            child: BlocListener<TenantDetailsPageBloc, TenantDetailsPageState>(
              ...
              child: StatefulBuilder(
                builder: (BuildContext context, StateSetter setState) {
                  return BlocBuilder<TenantDetailsPageBloc, TenantDetailsPageState>(
                    builder: (context, state) {
                      return Padding(
                        padding: EdgeInsets.only(
                          left: 16,
                        ...
                        child: Form(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              DropdownButtonFormField<int>(
                                decoration: const InputDecoration(
                                    labelText: 'Select Pending/Overdue Payment'),
                                value: selectedPendingPayment?.id,
                                items: consolidatedPendingPayments
                                    .map((p) => p.id)
                                    .toSet()
                                    .map((id) {
                                  final payment = consolidatedPendingPayments.firstWhere((p) => p.id == id);
                                  final isSecurityDeposit = payment.description?.toLowerCase().contains('deposit') ?? false;
                                  final isConsolidated = payment.description?.contains('Units:') ?? false;
                                ...
                                onChanged: (paymentId) {
                                  if (paymentId != null) {
                                    setState(() {
                                      selectedPendingPayment = consolidatedPendingPayments.firstWhere((p) => p.id == paymentId);
                                      if (!isPartialPayment) {
                                        amount = selectedPendingPayment!.amount.toDouble();
                                      }
                                    });
                                  }
                                },
                                ...
                                onPressed: state is TenantDetailsPageLoading
                                    ...
                                    : () {
                                        if (formKey.currentState!.validate()) {
                                          ...
                                          String paymentType = (selectedPendingPayment!.description?.toLowerCase().contains('deposit') ?? false)
                                              ? 'deposit'
                                              : 'rent';
                                          String key = '${paymentType}_${selectedPendingPayment!.dueDate}_${selectedPendingPayment!.amount}';
                                          
                                          if (groupedPayments.containsKey(key)) {
                                            allPaymentIds = groupedPayments[key]!.map((p) => p.id).toList();
                                          } else {
                                            allPaymentIds = [selectedPendingPayment!.id];
                                          }
                                          ...
                                          context
                                              ...
                                              .add(
                                                AddPaymentPressed(
                                                  tenantId: widget.tenant.id,
                                                  pendingPaymentId: allPaymentIds,
                                                  amount: isPartialPayment
                                                      ? amount.toInt()
                                                      : selectedPendingPayment!.amount.toInt(),
                                                  paymentDate:
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: _tenancies.any((t) => t.status.toLowerCase() == 'active') 
        ...
        child: Column(
          ...
          children: [
            ...
            if (hasDiscountGroups) ...[
              ..._discountGroups.map((discountGroup) => _buildDiscountGroupCard(discountGroup)),
              ...
              return ListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Text('Rent: ${NumberFormat.currency(symbol: 'Ksh').format(tenancy.monthlyRent)}'),
                      ],
                    ),
                    Text('Since: ${DateFormat('MMMM dd, yyyy').format(tenancy.startDate)}'),
  ...
  Widget _buildDiscountGroupCard(TenantDiscountGroup group) {
    ...
    final unitsInGroup = _units
        .where((u) => unitIdsInGroup.contains(u.id))
        .toList();
  ...
  Widget _buildDiscountBadge(TenantDiscountGroup group) {
    ...
    if (group.discountType == 'flat') {
      discountText = '- ${NumberFormat.currency(symbol: 'Ksh').format(group.discountValue)}';
    } else {
          ...
          child: BlocListener<TenantDetailsPageBloc, TenantDetailsPageState>(
            listener: (context, state) {
              if (state is TenantDetailsPageSuccess) {
                Navigator.pop(context); // Close the bottom sheet
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Units bundle created successfully.')),
                );
                // Reload home data
                context.read<HomeBloc>().add(LoadHome());
              } else if (state is TenantDetailsPageError) {
            ...
            child: StatefulBuilder(
              builder: (context, setState) {
                ...
                standardTotalRent = selectedUnits.isEmpty ? 0.0 : 
                          ...
                          children: [
                            ...
                            selectedUnits.isEmpty
                                ...
                                : Wrap(
                                    spacing: 8,
                                    runSpacing: 8,
                                    children: selectedUnits.map((unit) => Chip(
                                      label: Text('Unit ${unit.unitNumber}'),
                            ...
                            DropdownButtonFormField<int>(
                              ...
                              onChanged: (unitId) {
                                if (unitId != null) {
                                  ...
                                  final selectedUnit = availableUnits.firstWhere((u) => u.id == unitId);
                                  setState(() {
                        ...
                        if (selectedUnits.isNotEmpty) ...[
                                ...
                                child: RadioListTile<String>(
                                  ...
                                  onChanged: (value) {
                                    setState(() {
                                      ...
                                      discountValueController.text = discountType == 'flat' 
                                          ? suggestedDiscount.toStringAsFixed(0) 
                                          : '10';
                                    });
                                  },
                                ),
                          ...
                          TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              hintText: discountType == 'flat' ? 'e.g., 500' : 'e.g., 10',
                              prefixText: discountType == 'flat' ? 'Ksh ' : null,
                          ...
                          builder: (context, state) {
                            return SizedBox(
                              ...
                              child: ElevatedButton(
                                ...
                                onPressed: (selectedUnits.length < 2 || state is TenantDetailsPageLoading)
                                    ? null // Disable if fewer than 2 units or loading
                                    : () {
                                        if (formKey.currentState!.validate()) {
                                          final discountName = discountNameController.text.trim();
                                          final discountValue = double.parse(discountValueController.text.trim());
                                          final monthlyRent = double.parse(rentController.text.trim());
                                          final unitIds = selectedUnits.map((u) => u.id).toList();
  ...
  Widget _buildPaymentHistorySection(BuildContext context) {
    ...
    if (state is HomeLoaded) {
      payments = state.payments
          .where((payment) =>
              ...
              _tenancies.any((tenancy) => tenancy.id == payment.unitTenancyId) ||
              // OR include bundle payments linked to discount groups
              (payment.isBundlePayment == true && 
               _discountGroups.any((group) => group.id == payment.discountGroupId)))
          .toList();
    }
    ...
    for (var payment in payments) {
      ...
      String key = '${paymentType}_${payment.dueDate}_${payment.amount}_${status}_${paymentDateStr}';
    ...
    groupedPayments.forEach((key, payments) {
      ...
      if (payments.length > 1) {
        // Create a consolidated payment
        bool isDeposit = payments.first.description?.toLowerCase().contains('deposit') ?? false;
        ...
        final units = _units.where((u) => 
          _tenancies.any((t) => 
            unitTenancyIds.contains(t.id) && t.unitId == u.id
          )
        ).toList();
